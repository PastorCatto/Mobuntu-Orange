...
# (Using || true so the script doesn't fail if one was already enabled)
...
echo "--- 5. Cleaning Up Image Space ---"
...
echo "--- Entering Chroot to Execute Payload ---"
# 3. Execute the payload
chroot "$CHROOT_DIR" /bin/bash -c "/tmp/apply_fixes.sh"

echo "--- Tearing Down Chroot Environment ---"
# 4. Clean up the payload script and unmount safely
rm -f "$CHROOT_DIR/tmp/apply_fixes.sh"
...
echo "--- Success! Image is patched, unmounted, and ready to be packaged. ---"
