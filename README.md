...
4.  Starting with RC8, we intend to also implement an OTA Boot Updater, where the OS auto-flashes the new boot.img to the /boot partition, eliminating the need to flash a new Boot.img manually after updates! (thanks to arkadin91 for the OTA Script for initramfs)
